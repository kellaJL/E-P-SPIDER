...
class downloaderware:
    ...
    def Rem_Repate(self):
            ...
            for url1 in self.img.distinct('seed'):  # 使用distinct方法，获取每一个独特的元素列表
                num = self.img.count({"seed": url1})  # 统计每一个元素的数量
                print(num)
                for i in range(1, num):  # 根据每一个元素的数量进行删除操作，当前元素只有一个就不再删除
                    ...
                    self.img.remove({"seed": url1}, 0)
                for i in self.img.find({"seed": url1}):  # 打印当前所有元素
                    print(i)
    ...
    def downloaderware(self,url):
        ...
        for used_seed in seeds:
            self.img.remove({"seed":used_seed})
        return self.is_seed_empty
#url="http://588ku.com/"
#d=downloaderware()
