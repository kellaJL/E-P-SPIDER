...
class SpiderSchedule:
    ...
    def Rem_Repate(self):
            ...
            :return:
             """
            url = db.urldatabase618
            for url1 in url.distinct('url'):  # 使用distinct方法，获取每一个独特的元素列表
                num = url.count({"url": url1})  # 统计每一个元素的数量
                print(num)
                for i in range(1, num):  # 根据每一个元素的数量进行删除操作，当前元素只有一个就不再删除
                    ...
                    url.remove({"url": url1}, 0)
                for i in url.find({"url": url1}):  # 打印当前所有元素
                    print(i)
    ...
    def SpiderSchedule(self,start_url):
        ...
        all=url.find({"is_traveled":-1},{'url':1,'is_traveled':1})#在此处筛选未被探索过的url，使用bool标志
